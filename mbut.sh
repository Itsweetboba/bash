...
if [ "$OPSI" = "1" ]; then
    ...
    in_func == 1 && /^s*{/ {
        ...
        print "            throw new DisplayException("Lu Siapa Mau Delet User Lain Tolol? Izin Dulu Sama Id " admin_id " Kalo Mau Delete©Protect By @PaiiTheStory V'"'"'"$VERSION"'"'"')");";
        print "        }";
        in_func = 0; next;
    }
    { print }
    ' "${CONTROLLER_USER}.bak" > "$CONTROLLER_USER"
    echo -e "${GREEN}✔ Protect Delete User di UserController selesai.${RESET}"

    echo -e "${YELLOW}➤ Menambahkan Protect Update User...${RESET}"
    awk -v admin_id="$ADMIN_ID" '
    ...
    in_func == 1 && /^s*{/ {
        print;
        print "        $currentUserId = $request->user()->id;";
        print "        if ($currentUserId !== " admin_id ") {";
        print "            throw new DisplayException("Lu Siapa Mau Update User Lain Tolol? Izin Dulu Sama Id " admin_id " Kalo Mau Update©Protect By @PaiiTheStory V'"'"'"$VERSION"'"'"')");";
        print "        }";
        in_func = 0; next;
    }
    { print }
    ' "$CONTROLLER_USER" > "${CONTROLLER_USER}.tmp" && mv "${CONTROLLER_USER}.tmp" "$CONTROLLER_USER"
    echo -e "${GREEN}✔ Protect Update User di UserController selesai.${RESET}"
    ...
    in_func == 1 && /^s*{/ {
        ...
        print "            throw new DisplayException("Lu Siapa Mau Delet Server Lain Tolol? Izin Dulu Sama Id " admin_id " Kalo Mau Delet©Protect By @PaiiTheStory V'"'"'"$VERSION"'"'"')");";
        print "        }";
        in_func = 0; next;
    }
    { print }
    ...
    if yarn build:production --progress; then
        echo -e "${GREEN}🎉 Protect V$VERSION & Build Panel berhasil dipasang.${RESET}"
    else
        echo -e "${RED}❌ Build panel gagal. Silakan cek errornya.${RESET}"
        exit 1
    fi
    ...
    if yarn build:production --progress; then
        ...
        echo -e "${RED}❌ Build panel gagal saat restore.${RESET}"
        exit 1
    fi
